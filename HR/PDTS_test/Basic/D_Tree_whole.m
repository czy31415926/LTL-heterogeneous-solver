function [tree] = D_Tree_whole(T_whole,agent,task,B,T)
%% 决策树构造tree
%tic
finish=zeros([1,length(B.S)]);%已探索
finish(1)=1;
tree=[];
tree=[tree;struct('state',[],'ori',[],'dis',[],'finish',[],'traverse',[],'agents',[],'p_pre',[],'t_pre',[],'B',[],'T',[],'task_flag',[])];%新状态，根节点，距离,此序列的已更新状态，节点状态
tree(1).state=1;tree(1).ori=[];tree(1).dis=0;tree(1).finish=[1];tree(1).traverse=0;tree(1).agents=zeros([1,40]);
for i=1:length(agent)
    tree(1).p_pre=[tree(1).p_pre;agent(i).p_pre];
    tree(1).t_pre=[tree(1).t_pre;agent(i).t_pre];
end
tree(1).B=1;tree(1).T=1;tree(1).task_flag=0;
for ii=1:1000
    n=0;
    for i=1:length(tree)
                ...
                for k=1:T.N%遍历地点
                    ...
                    if ~isempty(find(cell2mat(B.trans(stateB,j))==ap))&&isempty(find(tree(i).finish==j))%存在可行转换且不在完成序列中
                        ...
                        elseif father.task_flag==son(end).B && father.task_flag>0%完成第一次后缀循环
                            son(end).task_flag=length(B.S)+1;son(end).traverse=1;
                        else
